#!/usr/bin/env python3
"""s22 — s21's rubric (calibration + familiar-name red flag + stroke verify)
but with `claude-fable-5` instead of Opus. Test whether Fable-5 has different
name-priming behavior than Opus on L35 and elsewhere.

Same tight cell crop, all 8 fixture rows. 16 calls, ~10-15 min.

Run:
    .venv/bin/python src/strategies/s22.py
"""
...
def fable_backend(jpg: Path, prompt: str, schema: dict) -> dict:
    """Claude via the `claude` CLI, pinned to claude-fable-5."""
...
def main() -> None:
    ...
    scratch = Path(__file__).parent / "_s22"
    ...
    print(f"s22: s21 rubric on {FABLE_MODEL} (no Opus) on {len(lines)} rows of "
          f"{reel} frame {frame:04d}")
    print(f"{'ln':>3} {'fld':<5} | {'Fable-5':<15} {'conf':<7} {'review':<7} | truth")
    print("-" * 68)
    ...
    for line in lines:
        ...
        for field, prompt, field_key in (
            ...
            print(f"L{line:02d} {field:<5} | "
                  ...
                  flush=True)
            row["fields"][field] = {"truth": truth, "fable_5": c}
        results.append(row)
