#!/usr/bin/env python3
"""s17 — try different Claude models on L35 (Opus vs Sonnet vs Haiku).

User's observation on s16: Claude went Mary → Marv → Marc as we added
abbreviation hints, but never landed on "Wm." — the models are cycling
through plausible male-name shapes without visually verifying against Wm.
Might be an Opus-specific quirk.

Test: same tight name-cell crop + same simple exact-transcription prompt
(like s2), but call three Claude variants — the current Opus baseline
(claude-opus-4-8), Sonnet 4.6 (claude-sonnet-4-6), Haiku 4.5
(claude-haiku-4-5-20251001). If any Claude variant reads "Wm." on L35,
we know it's an Opus quirk and can route L35-like cells to a different
Claude for confirmation.

L35 only. 3 models × 2 fields = 6 calls.

Run:
    .venv/bin/python src/strategies/s17.py
"""
...
import backends  # noqa: E402  (loads .env, provides _strip_fence + timeouts)

CLAUDE_MODELS = {
    "opus": backends.CLAUDE_MODEL,   # current baseline (claude-opus-4-8)
...
def claude_with_model(jpg: Path, prompt: str, schema: dict, model: str) -> dict:
    ...
    Same subprocess flags (--allowedTools Read --strict-mcp-config) + retry.
    """
...
def main() -> None:
    ...
    scratch = Path(__file__).parent / "_s17"
    ...
    print(f"s17: Claude Opus vs Sonnet vs Haiku on {TARGET_LINES} of "
