...
- **False-agreement finding (mechanism SOLVED, scoping open).** L33 surname: Claude originally read "Gibson", Gemini "Cissen"; at the nudge BOTH converged to "Cissen" (tier1, MEDIUM), which is what every strategy then output. The user confirms the paper says "Gibson" (3rd letter is a cursive `b`). So agreement overrode a correct read — a distinct failure from the invented blend, and one no debate strategy can fix (they only fire on DISagreement). Fix is upstream, and now **proven**: see the grapheme-decomposition finding below.
- **Grapheme-decomposition re-read — breaks the false-agreement blind spot (PROVEN on L33).** Root cause of the shared "Cissen" misread: both models do holistic *word* recognition, so a top-down language prior overrides the actual strokes — the "b" ascender gets read as an archaic long-s (ſ), giving a phantom `ss`, and once anchored on capital "C" (vs the true "G") the rest is reshaped into a plausible surname. Fix: on a hard name cell, take a **tight high-zoom crop of just that name** and prompt the model to **describe each letter's physical STRUCTURE first** (tall ascender? does the stroke close into a baseline bowl → `b`, or stay open → long-s? descender? are adjacent letters the SAME or DIFFERENT shape?) and only THEN assemble the word. This suppresses the word-prior and forces bottom-up reading. Results on L33 (surname crop): **Claude → "Gibson" (correct)**, citing "closes into a baseline bowl → b, not long-s"; **Gemini → "Ogden" (still wrong, but NO longer "Cissen")**. The key win is not that one model is now perfect — it's that the two **no longer share the blind spot**, so the false agreement becomes a real DISagreement the existing debate can resolve. End-to-end proof: feeding `Gibson` vs `Ogden` into the **unchanged v4** debate resolves to **"Gibson" [MEDIUM, tier1]** — it agrees at the nudge, i.e. once "Gibson" is merely ON THE TABLE both models re-read the full row and converge. So the blind spot was never inability to read the glyph; it was that neither model ever *proposed* the right candidate. Decomposition's whole job is to get the correct candidate onto the ballot; the existing machinery converges from there. (Scratch PoC only so far — not yet wired into `src/`; the open question is which cells to trigger it on — see Open issues.)
- **Claude CLI is the scaling bottleneck, not the debate's call count.** v4 fires only **2** model calls when the nudge agrees (tier1) and **6** only on the full defend→decide path — nothing in between. The L33 run resolved at tier1 (2 calls) yet was slow: each `claude -p` vision read is 40–90s normally and, if one hangs, burns the 180s timeout + up to 3 retries (~9 min worst case for ONE call). The latency lever for scale is the Claude CLI backend (parallelize / replace-with-API), not trimming debate stages.

## Open issues / gotchas

- **False agreement on hard glyphs — mechanism solved, scoping/wiring open.** When both models mis-read the same ambiguous glyph identically (L33 "Gibson"→both "Cissen"), consensus + adjudication give false confidence — they only act on DISagreement. The **grapheme-decomposition re-read breaks this** (proven on L33 → "Gibson"; see Findings), but it's a scratch PoC, not yet in `src/`. Two open pieces: (1) **which cells trigger it** — we can't run a tight-crop decomposition pass on every cell of ~590 pages, so we need a signal (candidate: run it on all *surnames*, or only cells a legibility heuristic flags); (2) **wire it into the pipeline** — cleanest fit is a pre-adjudication step that, on flagged cells, replaces one/both agents' reading with the decomposition read so any new disagreement flows into the existing v4 debate. Decision pending with the user.
